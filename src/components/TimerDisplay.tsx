...
const TimerDisplay: React.FC<TimerDisplayProps> = ({ formatTime, time, maxTime, isBreak }) => {
	...
	return (
		<Box position="relative" display="flex" flexDirection="column" alignItems="center">
			<Circle size="300px" bg={bgColor} position="relative" mt={4} onClick={handleCircleClick} cursor="pointer">
				{animationMode === "vertical" ? (
					<Circle size="280px" bg={progressColor} position="absolute" clipPath={`inset(${100 - (time / maxTime) * 100}% 0 0 0)`} />
				) : (
					<Box position="absolute" width="280px" height="280px" display="flex" alignItems="center" justifyContent="center">
						<svg width="280" height="280" viewBox="0 0 280 280" style={{ transform: "rotate(-90deg)", position: "absolute" }}>
							{/* Background Circle */}
							<circle cx="140" cy="140" r={radius} fill="transparent" stroke={progressColor} strokeWidth="140" strokeDasharray={dashArray} strokeDashoffset={dashOffset} />
						</svg>
					</Box>
				)}
			...
			<HStack mt={4} bg={segmentBg} p={1} borderRadius="full" spacing={0}>
				<Box
					as="button"
					onClick={(e: React.MouseEvent) => {
						e.stopPropagation();
						setAnimationMode("vertical");
					}}
			...
			<ColorPickerComponent colorPick={colorPick} setColorPick={setColorPick} pickerVisible={pickerVisible} setPickerVisible={setPickerVisible} />
		</Box>
	);
};
